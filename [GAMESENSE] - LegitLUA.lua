...
function LegitFunction()
    ...
    if ui.get(FOV_Checkbox) == true then
        renderer.circle_outline(x / 2, y / 2, r, g, b, a, ui.get(FOV), 1, 1, 1)
    end
    if (Flags == 257) and ui.get(Aircheck_Checkbox) == true then
        ui.set(Aimbot, true)
    elseif (Flags == 256) and ui.get(Aircheck_Checkbox) == true then
        ui.set(Aimbot, false)
    end
end
client.set_event_callback('paint', LegitFunction)
